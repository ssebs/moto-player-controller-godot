...
## Sets player_controller.state.speed from get_power_output + effective_max_speed
func _handle_acceleration(delta):
    ...
    var effective_max_speed = player_controller.bike_tricks.get_effective_max_speed()
...
func _handle_deceleration(delta):
    ...
    if player_controller.bike_input.front_brake > 0 or player_controller.bike_input.rear_brake > 0:
        var front_effectiveness = 0.6 if player_controller.bike_tricks.is_front_wheel_locked() else 1.0
    ...
    if player_controller.bike_input.throttle == 0:
        ...
        var drag = br.friction * (0.5 + clutch_engagement * 0.5) + br.engine_brake_strength * player_controller.state.rpm_ratio * clutch_engagement
        player_controller.state.speed = move_toward(player_controller.state.speed, 0, drag * delta)

## Lean angle from player input. Lean directly controls both visual tilt and steering. Sets player_controller.state.lean_angle
...
func _handle_countersteering(_delta):
    print("_handle_countersteering does nothing atm")
