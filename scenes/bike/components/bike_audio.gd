...
func _bike_setup(p_controller: PlayerController):
    ...
    # Gearing signals
    player_controller.bike_gearing.gear_grind.connect(_play_gear_grind)
    player_controller.bike_gearing.gear_changed.connect(_on_gear_changed)
    player_controller.bike_gearing.engine_stalled.connect(_stop_engine)

    # Tricks signals
    player_controller.bike_tricks.tire_screech_start.connect(_play_tire_screech)
    player_controller.bike_tricks.tire_screech_stop.connect(_stop_tire_screech)
    player_controller.bike_tricks.boost_started.connect(play_nos)
    player_controller.bike_tricks.boost_ended.connect(_stop_nos)
...
func _bike_update(delta):
    match player_controller.state.player_state:
        ...
        _:
            _update_engine_audio(delta, player_controller.state.rpm_ratio)
...
func _play_tire_screech(volume: float):
...
func _update_engine_audio(delta: float, rpm_ratio: float):
    if !player_controller.engine_sound:
        return
